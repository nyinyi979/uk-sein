...
export default function SignUpBack({
  onClick,
  firstPage,
}: {
  onClick: () => void;
  ...
  return (
    <div className="md:hidden flex flex-col gap-1 text-sm">
      <button onClick={onClick} className="back-arr-text flex">
        <svg
          width="19"
          height="16"
          viewBox="0 0 25 21"
        ...
        <span className="font-semibold">
          {firstPage
            ? t("back-to.back-to-home")
            : t("back-to.back-to-personal")}
        </span>
      </button>
      <p>
        <span className="text-grey-100">${t("main-pages.home")} - </span>{" "}
        {firstPage
          ? `${t("signup.sign-up")} - ${t("signup.personal")}`
          : `${t("signup.sign-up")} - ${t("signup.shipping-info")}`}
      </p>
    </div>
  );
}
