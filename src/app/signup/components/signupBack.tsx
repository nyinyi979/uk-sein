export default function SignUpBack({onClick,firstPage}:{
  onClick: ()=>void;
  firstPage: boolean
}) {
  return (
    <div className="md:hidden flex flex-col gap-1 text-sm">
      <button onClick={onClick} className="back-arr-text flex">
        ...
        <span className="font-semibold">Back {firstPage ? "Home" : "To Personal Information"}</span>
      </button>
      <p>
        <span className="text-grey-100">Home - </span> {firstPage? "Sign up - Personal Information" : "Sign up - shipping information"}
      </p>
    </div>
  );
}
