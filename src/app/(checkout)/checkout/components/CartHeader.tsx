    ...
    <div className="xl:hidden flex flex-row">
      ...
      >
        ...
        <span className="font-sora font-semibold text-sm">
          {page === "Shipping"
            ? t("back-to.back-to-cart")
            : t("back-to.back-to-shipping")}
        </span>
      </button>
      <button
        onClick={openCart}
        className="w-fit ml-auto py-2 px-3 rounded-[5px] bg-khaki-500 font-semibold text-[10px] text-white hover:bg-khaki-700 duration-300"
      >
        {t("checkout.view-cart-summary")}
      </button>
    </div>
  );
}
